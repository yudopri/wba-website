...
class BpjsController extends Controller
{
     ...
     public function index(Request $request)
    {
        ...
        $transaksi = Bpjs::with('user')
            ->whereBetween('created_at', [$tanggalAwal, $tanggalAkhir])
            ->orderBy('created_at', 'desc')
            ->get();

        $saldo = Bpjs::sum('debit') - Bpjs::sum('kredit');
        $totalPengeluaran = Bpjs::whereBetween('created_at', [$tanggalAwal, $tanggalAkhir])->sum('kredit');

        return view('admin.bpjs.index', compact(
            'transaksi',
            'saldo',
            'totalPengeluaran',
            'range'
...
$validated['id_user'] = auth()->id();
        ...
        $lastSaldo = Bpjs::orderBy('created_at', 'desc')->value('saldo') ?? 0;
